...
export const DEFAULT_DEV_STATE_DIR = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".t3sparks", "dev"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@t3sparks/contracts",
    "--filter=@t3sparks/web",
    "--filter=t3sparks",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=t3sparks"],
  "dev:web": ["run", "dev", "--filter=@t3sparks/web"],
  "dev:desktop": ["run", "dev", "--filter=@t3sparks/desktop", "--filter=@t3sparks/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("T3SPARKS_PORT_OFFSET"),
  devInstance: optionalStringConfig("T3SPARKS_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid T3SPARKS_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `T3SPARKS_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric T3SPARKS_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed T3SPARKS_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      T3SPARKS_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://localhost:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      T3SPARKS_STATE_DIR: resolvedStateDir,
    };

    if (host !== undefined) {
      output.T3SPARKS_HOST = host;
    }

    if (authToken !== undefined) {
      output.T3SPARKS_AUTH_TOKEN = authToken;
    } else {
      delete output.T3SPARKS_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.T3SPARKS_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.T3SPARKS_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.T3SPARKS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.T3SPARKS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.T3SPARKS_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.T3SPARKS_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.T3SPARKS_MODE = "web";
      delete output.T3SPARKS_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read T3SPARKS_PORT_OFFSET/T3SPARKS_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("T3SPARKS_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "T3SPARKS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("T3SPARKS_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.T3SPARKS_PORT)} webPort=${String(env.PORT)} stateDir=${String(env.T3SPARKS_STATE_DIR)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  stateDir: Flag.string("state-dir").pipe(
    Flag.withDescription("State directory path (forwards to T3SPARKS_STATE_DIR)."),
    Flag.withFallbackConfig(optionalStringConfig("T3SPARKS_STATE_DIR")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to T3SPARKS_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("T3SPARKS_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to T3SPARKS_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("T3SPARKS_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to T3SPARKS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("T3SPARKS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to T3SPARKS_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("T3SPARKS_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to T3SPARKS_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("T3SPARKS_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to T3SPARKS_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("T3SPARKS_PORT")),
  ),
