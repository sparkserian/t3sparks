...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit T3SPARKS_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "T3SPARKS_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid T3SPARKS_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults state dir to ~/.t3sparks/dev when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3SPARKS_STATE_DIR, defaultStateDir);
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3SPARKS_STATE_DIR, resolve("/tmp/override-state"));
        assert.equal(env.T3SPARKS_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.T3SPARKS_NO_BROWSER, "1");
        assert.equal(env.T3SPARKS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.T3SPARKS_LOG_WS_EVENTS, "1");
        assert.equal(env.T3SPARKS_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            T3SPARKS_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          stateDir: undefined,
        ...
        assert.equal(env.T3SPARKS_MODE, "web");
        assert.equal(env.T3SPARKS_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3SPARKS_LOG_WS_EVENTS, "0");
      }),
    );
  });
