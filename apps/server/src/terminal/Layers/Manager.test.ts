...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "t3sparks-terminal-"));
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.T3SPARKS_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          T3SPARKS_PROJECT_ROOT: "/repo",
          T3SPARKS_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.T3SPARKS_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.T3SPARKS_WORKTREE_PATH).toBe("/repo/worktree-a");
