...
import { resolveAutoFeatureBranchName, sanitizeFeatureBranchName } from "@t3sparks/shared/git";
...
export const makeGitManager = Effect.gen(function* () {
  ...
  const resolveCommitAndBranchSuggestion = (input: {
    cwd: string;
    branch: string | null;
    provider?: "codex" | "claudeAgent" | "gemini" | "githubCopilot";
    model?: string;
    commitMessage?: string;
  ...
  const runPrStep = (
    ...
    Effect.gen(function* () {
      ...
      const bodyFile = path.join(tempDir, `t3sparks-pr-body-${process.pid}-${randomUUID()}.md`);
      yield* fileSystem
