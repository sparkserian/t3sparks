...
describe("WebSocket Server", () => {
  ...
  async function createTestServer(
    ...
    const stateDir = options.stateDir ?? makeTempDir("t3sparks-ws-state-");
  ...
  it("serves persisted attachments from stateDir", async () => {
    const stateDir = makeTempDir("t3sparks-state-attachments-");
  ...
  it("serves persisted attachments for URL-encoded paths", async () => {
    const stateDir = makeTempDir("t3sparks-state-attachments-encoded-");
  ...
  it("serves static index for root path", async () => {
    const stateDir = makeTempDir("t3sparks-state-static-root-");
    const staticDir = makeTempDir("t3sparks-static-root-");
  ...
  it("rejects static path traversal attempts", async () => {
    const stateDir = makeTempDir("t3sparks-state-static-traversal-");
    const staticDir = makeTempDir("t3sparks-static-traversal-");
  ...
  it("includes bootstrap ids in welcome when cwd project and thread already exist", async () => {
    const stateDir = makeTempDir("t3sparks-state-bootstrap-existing-");
  ...
  it("responds to server.getConfig", async () => {
    const stateDir = makeTempDir("t3sparks-state-get-config-");
  ...
  it("bootstraps default keybindings file when missing", async () => {
    const stateDir = makeTempDir("t3sparks-state-bootstrap-keybindings-");
    const keybindingsPath = path.join(stateDir, "keybindings.json");
  ...
  it("falls back to defaults and reports malformed keybindings config issues", async () => {
    const stateDir = makeTempDir("t3sparks-state-malformed-keybindings-");
    const keybindingsPath = path.join(stateDir, "keybindings.json");
  ...
  it("ignores invalid keybinding entries but keeps valid entries and reports issues", async () => {
    const stateDir = makeTempDir("t3sparks-state-partial-invalid-keybindings-");
    const keybindingsPath = path.join(stateDir, "keybindings.json");
    fs.writeFileSync(
      keybindingsPath,
      JSON.stringify([
  ...
  it("pushes server.configUpdated issues when keybindings file changes", async () => {
    const stateDir = makeTempDir("t3sparks-state-keybindings-watch-");
    const keybindingsPath = path.join(stateDir, "keybindings.json");
    fs.writeFileSync(keybindingsPath, "[]", "utf8");
  ...
  it("reads keybindings from the configured state directory", async () => {
    const stateDir = makeTempDir("t3sparks-state-keybindings-");
    const keybindingsPath = path.join(stateDir, "keybindings.json");
    fs.writeFileSync(
      keybindingsPath,
      JSON.stringify([
  ...
  it("upserts keybinding rules and updates cached server config", async () => {
    const stateDir = makeTempDir("t3sparks-state-upsert-keybinding-");
    const keybindingsPath = path.join(stateDir, "keybindings.json");
    fs.writeFileSync(
      keybindingsPath,
  ...
  it("routes terminal RPC methods and broadcasts terminal events", async () => {
    const cwd = makeTempDir("t3sparks-ws-terminal-cwd-");
  ...
  it("catches websocket message handler rejections and keeps the socket usable", async () => {
    ...
    try {
      ...
      const workspace = makeTempDir("t3sparks-ws-handler-still-usable-");
  ...
  it("supports projects.searchEntries", async () => {
    const workspace = makeTempDir("t3sparks-ws-workspace-entries-");
  ...
  it("supports projects.writeFile within the workspace root", async () => {
    const workspace = makeTempDir("t3sparks-ws-write-file-");
  ...
  it("rejects projects.writeFile paths outside the workspace root", async () => {
    const workspace = makeTempDir("t3sparks-ws-write-file-reject-");
