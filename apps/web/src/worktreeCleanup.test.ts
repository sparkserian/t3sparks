...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.t3sparks/worktrees/t3sparks-mvp/t3sparks-4e609bb8",
    );
    expect(result).toBe("t3sparks-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.t3sparks\\worktrees\\t3sparks-mvp\\t3sparks-4e609bb8",
    );
    expect(result).toBe("t3sparks-4e609bb8");
  });

  it("uses the final segment even when outside ~/.t3sparks/worktrees", () => {
