...
export default function ConvexControl({
  ...
  const { data: status } = useQuery(convexStatusQueryOptions({ environmentId, cwd }));
  ...
  useEffect(() => {
    ...
    return api.terminal.onEvent((event) => {
      ...
      if (
        event.terminalId !== CONVEX_DEV_TERMINAL_ID &&
        event.terminalId !== CONVEX_TASK_TERMINAL_ID
      ) {
        return;
      }
      ...
      if (event.type === "exited") {
        ...
        void queryClient.invalidateQueries({
          queryKey: convexQueryKeys.status(environmentId, cwd),
        });
      }
    });
  }, [cwd, environmentId, queryClient, threadId]);
