...
- Dev commands default `T3SPARKS_STATE_DIR` to `~/.t3sparks/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --state-dir ~/.t3sparks/another-dev-state`
...
Set `T3SPARKS_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `T3SPARKS_DEV_INSTANCE`)
- Example: `T3SPARKS_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `T3SPARKS_PORT_OFFSET` to a numeric offset.
